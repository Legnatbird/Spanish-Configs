...
function create_name_and_role_container(name, roles) {
	...
	for (let role of roles) {
		if (role == "Unknown" || role == "Desconocido" || role == "desconocido"){
			continue
		}
...
function create_rank_information(wins, losses, rank, lps) {
	...
	const winCount = create_element("span", "win-revealer", `Victorias <b>${wins}</b>`)
	const lossCount = create_element("span", "loss-revealer", `Derrotas <b>${losses}</b>`)
...
async function scrape_leagueofgraphs(players, region) {
	...
	await fetch(`https://porofessor.gg/partial/es/pregame-partial/${region}/${list_players}/ranked-only`).then(response => response.text())
		.then(async data => {
			...
			if (!doc.querySelector(".cards-list.cards-list-centered")){
				console.log(doc, `https://porofessor.gg/partial/es/pregame-partial/${region}/${list_players}/ranked-only`)
			}
			if (doc.querySelector(".cards-list.cards-list-centered")){
				for (let player_card of doc.querySelector(".cards-list.cards-list-centered").children){
					...
					catch {result.rank = "Sin clasificar"}
					...
					catch { result.rank_last_season = "Sin clasificar" }
...
async function create_reveal_box(target) {
	...
	title.innerText = "Emparejamiento";
